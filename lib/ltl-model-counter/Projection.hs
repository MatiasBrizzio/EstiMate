...
ltl2string expr = case expr of 
                        (Prop a) -> a
                        (T) -> "True"
                        (F) -> "False"
                        (Neg  e) -> "~"++(ltl2string e)
                        (And (e, e')) -> "("++ (ltl2string e) ++ ")"++"&"++"("++ (ltl2string e') ++")"
                        (Or (e, e')) -> "("++ (ltl2string e) ++ ")"++"|"++"("++ (ltl2string e') ++")"
                        (Imp (e, e')) -> "("++ (ltl2string e) ++ ")"++"->"++"("++ (ltl2string e') ++")"
                        (Equiv (e, e')) -> "("++ (ltl2string e) ++ ")"++"="++"("++ (ltl2string e') ++")"
                        (Next  e) -> "X"++(ltl2string e)
                        (Until (e, e')) -> "("++ (ltl2string e) ++ ")"++"U"++"("++ (ltl2string e') ++")"
                        (WUntil (e, e')) -> "("++ (ltl2string e) ++ ")"++"W"++"("++ (ltl2string e') ++")"
                        (Rel (e, e')) -> "("++ (ltl2string e) ++ ")"++"R"++"("++ (ltl2string e') ++")"
                        (MRel (e, e')) -> "("++ (ltl2string e) ++ ")"++"M"++"("++ (ltl2string e') ++")"
                        (Diam  e) -> "F"++(ltl2string e)
                        (Box  e) -> "G"++(ltl2string e)
...
-- Function
isSat :: String -> IO (Bool) 
isSat phi =   do
                    r <- createProcess (proc "./../aalta" [phi]) { std_out = CreatePipe, std_in = CreatePipe }
...
-- reduceProp :: ExpLTL a -> ExpLTL b
-- reduceProp a = if isSat (e) == "sat" then T

-- shall we change the signature?
-- \psi \setq \phi?
-- P 
-- Guarantees -> Mode -> new Guarantees.
-- proj :: ExpLTL phi -> ExpLTL mode -> ExpLTL projectedf
-- proj (Prop phi) mode  = if (isSat (ltl2string (Prop phi)) ++ " && " ++ mode) -- phi && mode \not\models TRUE iff phi is another mode.
-- 						then (Prop phi) else 
-- proj (Neg e) _  =  Neg e
-- proj (And(e,e')) = Or(proj e, proj e')
-- proj (Or(e,e')) = And(proj e, proj e')
-- proj (Imp(e,e')) = And(pnf e, proj e')
-- proj (Equiv(e,e')) = Or(And(pnf e, proj e'), And(proj e, pnf e'))
-- proj (Next e) = Next(proj e)
-- proj (Until(e,e')) = Rel(proj e, proj e')
-- proj (WUntil(e,e')) = proj (Or(Box (e), Until(e, e')))
-- proj (Rel(e,e')) = Until(proj e, proj e')
-- proj (Box e) = Until(T,proj e)
-- proj (MRel(e,e')) = error "Projection: Formula is not in Safety-LTL."
-- proj (Diam e) = error "Projection: Formula is not in Safety-LTL."
...
main = do
         ...
         if length formulas == 2 then
            let bc = unfoldBC (Data.List.last formulas) 1 ;
                phi = And (form,bc) ;
                toSAT = bltl2sat phi alphabet 1 ;
                --cnfSAT = cnf toSAT ;
                satString = (sat2string toSAT) 
            in
              --(trace ("bc " ++ show bc))
              writeFile (outfile++"-k1"++".sat") satString 
         else
            let toSAT = (bltl2sat form alphabet 1) ;
                cnfSAT = cnf toSAT ;
